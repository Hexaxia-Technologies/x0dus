...
.PARAMETER DestinationPath
    The folder where the backup will be stored. The script prompts to create it if needed.
...
function Confirm-DestinationCreation {
    param(
        [Parameter(Mandatory = $true)]
        [string]$Path
    )

    Write-Host "Destination directory '$Path' does not exist." -ForegroundColor Yellow

    while ($true) {
        $response = Read-Host "Create it now? (Y/N, default Y)"
...
function Initialize-Destination {
    ...
    if (-not (Test-Path -LiteralPath $fullPath)) {
        if (-not (Confirm-DestinationCreation -Path $fullPath)) {
            throw "Destination path '$fullPath' does not exist and was not created."
        }
